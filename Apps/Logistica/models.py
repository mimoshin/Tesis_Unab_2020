...
def select_month_filter(month,year):
    if month <10:
        month_filter = r'{0}-0{1}-.*'.format(str(year),str(month))
    else:
...
class Calendar_Factory():
    """
        Obtener total de eventos del mes (month - year)
    """
    @staticmethod
    def get_events(month,year):
        month_filter = select_month_filter(month,year)
    ...
    def create_event(e_type,e_re,u_admin,event):
        """
        Calendarizacion de evento dependiente de una solicitud e independiente de una solicitud
        """
        if e_type == 'Ind':
